...
```{r}    
    ...
    mx5int <- int5_avg[int5_avg$steps == max(int5_avg$steps),]$interval
```

The `r mx5int` interval is the 5-minute interval (averaged across all the days) that contains the maximum number of steps. 
